...
/*
 * @brief check eviction policy
 * @param[in] ctrl_fd file descriptor of opened control utility
 * @param[in] cache_id id of cache device
 * @param[out] mode mode identifier as integer
 * @return exit code of successful completion is 0; nonzero exit code means failure
 */
int get_eviction_policy(int ctrl_fd, unsigned int cache_id, int *policy)
{
	struct kcas_cache_info cmd_info;
	...
	*policy = cmd_info.info.eviction_policy;
	return SUCCESS;
}
...
int set_eviction_policy(unsigned int policy, unsigned int cache_id)
{
	...
	int orig_mode;
	int old_policy;
	struct kcas_set_cache_eviction_policy cmd;
	bool flush;
	...
	if (get_cache_mode(fd, cache_id, &orig_mode) || get_eviction_policy(fd, cache_id, &old_policy)) {
		cas_printf(LOG_ERR, "Error while retrieving cache properties.\n");
		close(fd);
		return FAILURE;
	}
	...
	if(old_policy == policy) {
		cas_printf(LOG_INFO, "New policy is the same as old policy. No changes required\n");
		close(fd);
		return SUCCESS;
	}
	 ...
	 * out of lazy write cache mode (like WB or WO), in which case user must explicitly
	 * state his preference */
	flush = ocf_mngt_cache_mode_has_lazy_write(orig_mode);
	...
	memset(&cmd, 0, sizeof(cmd));
	cmd.cache_id = cache_id;
	cmd.eviction_policy = policy;
	cmd.flush_data = flush;

	if (run_ioctl_interruptible_retry(fd, KCAS_IOCTL_SET_EVICTION_POLICY, &cmd, "Setting eviction policy",
			cache_id, OCF_CORE_ID_INVALID) < 0) {
		close(fd);
		if (OCF_ERR_FLUSHING_INTERRUPTED == cmd.ext_err_code) {
			assert(flush);
			...
			cas_printf(LOG_ERR, "Error while setting eviction policy for cache %d\n",
				cache_id);
			print_err(cmd.ext_err_code);
			return FAILURE;
		}
